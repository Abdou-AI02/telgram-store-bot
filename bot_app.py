...
async def create_sample_products():
    ...
    if r and r["c"] == 0:
        await conn.execute(
            "INSERT INTO products (name, price, stock, category, description, file_url) VALUES (?,?,?,?,?,?)",
...
async def create_user_if_not_exists(user_id: int, first_name: str, referred_by_id: int = None):
    ...
    if not await cur.fetchone():
        ...
        if referred_by_id:
            await conn.execute(
                "UPDATE users SET referrals = referrals + 1, points = points + ? WHERE user_id = ?", 
...
async def list_products(category: str = None):
    conn = await _get_conn()
    if category:
        cur = await conn.execute("SELECT * FROM products WHERE category = ? ORDER BY product_id", (category,))
    else:
...
async def get_all_categories():
    conn = await _get_conn()
    cur = await conn.execute("SELECT DISTINCT category FROM products")
    rows = await cur.fetchall()
    await conn.close()
...
async def add_product_db(name: str, price: float, stock: int, category: str, description: str, file_url: str):
    conn = await _get_conn()
    await conn.execute("INSERT INTO products (name, price, stock, category, description, file_url) VALUES (?, ?, ?, ?, ?, ?)", 
...
async def edit_product_db(product_id: int, name: str, price: float, stock: int, category: str, description: str):
    conn = await _get_conn()
    await conn.execute("UPDATE products SET name=?, price=?, stock=?, category=?, description=? WHERE product_id=?", 
...
async def get_total_sales_db():
    conn = await _get_conn()
    cur = await conn.execute("SELECT SUM(total) FROM orders WHERE status='مقبول ✅'")
    total_sales = await cur.fetchone()
    await conn.close()
    return total_sales[0] if total_sales and total_sales[0] else 0

async def get_total_orders_db():
    conn = await _get_conn()
    cur = await conn.execute("SELECT COUNT(order_id) FROM orders")
    total_orders = await cur.fetchone()
    await conn.close()
    return total_orders[0] if total_orders and total_orders[0] else 0
...
async def cmd_daily_tasks(message: types.Message):
    ...
    last_daily_task = user_data.get('last_daily_task')
    if last_daily_task:
        from datetime import datetime, timedelta
        last_daily_task_date = datetime.fromisoformat(last_daily_task)
...
async def process_delete_category(message: types.Message, state: FSMContext):
    ...
    conn = await _get_conn()
    await conn.execute("DELETE FROM products WHERE category = ?", (category_name,))
    await conn.commit()
    await conn.close()
...
async def process_notification_message(message: types.Message, state: FSMContext):
    ...
    if target == 'all':
        ...
        cur = await conn.execute("SELECT user_id FROM users ORDER BY created_at DESC LIMIT 10")
